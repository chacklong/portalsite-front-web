                ...
                <card shadow class="card-profile mt--300" no-body>
                    ...
                    <div class="mt-5 py-5 text-center">
                        <div v-for="(item,index) in newList" :key="index" class="row justify-content-center">
                            <!-- <div class="card-profile-image">
                            <a href="#">
                                <img src="http://192.168.31.30:1337/uploads/demo4_420c4d1deb.png" alt=" news image" width="700px" height="250px">
                            </a>
                            </div> -->
                            <div class="col-lg-9">
                                <h4>{{ item.attributes.newtitle }}</h4>
                                <p class="display2">{{ limitedContent }}</p>
                                <!-- <a href="#">Show more</a> -->
                                <router-link :to="{ name: 'NewsDetail', params: { id: item.id } }">Show more</router-link>
                            </div>
                        </div>
                    </div>
            </div>
...
export default {
    ...
    computed:{
        limitedContent(){
            const maxLength = 100;
            if(this.newList.length > 0 && this.newList[0].attributes.newcontent.length > maxLength){
                return this.newList[0].attributes.newcontent.slice(0,maxLength)+'...'
            }else{
                return this.newList[0].attributes.newcontent
            }
        }
    },
    methods: {
